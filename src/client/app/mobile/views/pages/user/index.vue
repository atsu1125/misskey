		...
		<header>
			...
			<div class="body">
				<div class="top">
					...
					<mk-follow-button v-if="!$store.getters.isSignedIn || (($store.getters.isSignedIn && $store.state.i.id != user.id) && !user.isBlocking)" :user="user" :key="user.id"/>
				</div>
				<div class="title">
					...
					<x-user-badges :user="user" :key="user.id" :with-roles="true"/>
